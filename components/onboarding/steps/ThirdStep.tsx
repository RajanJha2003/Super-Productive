...
const ThirdStep = () => {
  ...
  return (
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='w-full max-w-md space-y-8 mt-12'>
        <div className='space-y-1.5'>
          <FormField name='workspaceName' control={form.control} render={({field})=>(
            <FormItem>
              <FormLabel className='text-muted-foreground'>{t("THIRD_STEP.INPUTS.NAME")}</FormLabel>
              <FormControl>
                <Input className='bg-muted' placeholder={t("THIRD_STEP.PLACEHOLDERS.NAME")} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
