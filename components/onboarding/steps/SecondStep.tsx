...
const SecondStep = () => {
  ...
  const onSubmit = async (data: AdditionalUserInfoSecondPart) => {
      ...
      <div className="max-w-md w-full space-y-8 mt-14">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <FormControl>
                    ...
                    >
                      {useCases.map((useCase) => (
                        <FormItem
                          key={useCase.case}
                          className={`flex items-center space-x-3 space-y-0 p-3 rounded-md transition-colors duration-200 relative overflow-hidden ${form.getValues(
                            "useCase"
                          )===useCase.case?"bg-primary/40":"hover:bg-primary/10"}`}
                        >
                           <FormControl
                            onClick={(e) => {
            ...
            <Button type="submit" className="mt-10 w-full max-w-md font-semibold">
              {
                t("NEXT_BTN")
